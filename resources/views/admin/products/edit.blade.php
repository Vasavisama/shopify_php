...
@section('content')
    ...
    <div class="mt-8">
        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            @csrf
            @method('PUT')
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="quantity" name="quantity" type="number" value="{{ $product->inventory->quantity ?? 0 }}" required>
            </div>
                    ...
                    <div class="variant-group border-t pt-4 mt-4">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input type="text" name="variants[{{ $index }}][name]" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700" value="{{ $variant->name }}">
                            </div>
                            <div>
                                <label class="block text-gray-700 text-sm font-bold mb-2">Variant Value (e.g., Large)</label>
                                <input type="text" name="variants[{{ $index }}][value]" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700" value="{{ $variant->value }}">
                            </div>
                            <div>
                                <label class="block text-gray-700 text-sm font-bold mb-2">Price Modifier</label>
                                <input type="number" step="0.01" name="variants[{{ $index }}][price_modifier]" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700" value="{{ $variant->price_modifier }}">
                            </div>
                        </div>
